...
public:
        ...
        Args&&... _value)
    {
        ...
        {
            m_values[dense] = std::forward<Args>(_value);
            return m_values[dense];
        }

        return Emplace(_handle, std::forward<Args>(_value));
    }
    ...
    [[nodiscard]] std::span<const HandleT> GetHandles() const
    {
        return { m_handles };
    }
...
private:
    ...
    Vector<HandleT> m_handles = {};   // 핸들을 연속적으로 저장
