...
SerializeResult<JsonReader> JsonReader::FindField(
    const StringView _key) const
{
    ...
    if (!pField)
    {
        ...
        return Failed { eSerializerError::MissingField };
    }
    ...
    const StringView _key) const
{
    ...
    return yyjson_obj_getn(m_pValue, _key.data(), _key.size()) != nullptr;
}
...
SerializeResult<bool> JsonReader::Read_(
...
SerializeResult<int64_t> JsonReader::Read_(
...
SerializeResult<uint64_t> JsonReader::Read_(
...
SerializeResult<double> JsonReader::Read_(
...
SerializeResult<StringView> JsonReader::Read_(
...
SerializeResult<String> JsonReader::Read_(
...
SerializeResult<const char*> JsonReader::Read_(
...
SerializeResult<JsonDeserializer> JsonDeserializer::LoadFromString(
    const StringView             _json,
    const Flags<eJsonLoadOption> _options)
{
    ...
    if (!pDoc)
    {
        ...
        return Failed { eSerializerError::ParseFailed };
    }
...
SerializeResult<JsonDeserializer> JsonDeserializer::LoadFromFile(
    ...
    const Flags<eJsonLoadOption> _options)
{
    ...
    if (!file)
    {
        JUG_CORE_LOG_ERROR("JsonDeserializer: failed to open the given file");
        return Failed { eSerializerError::FileError };
    }
