#pragma once
#include <cstddef>
#include <initializer_list>
#include <memory>
#include <type_traits>
#include <utility>
#include <vector>

#include "Config.h"

namespace jug
{

template<typename T>
class RingBuffer
{
    // !std::is_default_constructible_v<T>를 만족하지 않는 타입을 위해 union을 사용
    union ITEM
    {
        T value;
    };

public:
    // ===========================================
    //  Iterator
    // ===========================================

    template<bool kbIsConst>
    class BaseIterator
    {
        template<bool>
        friend class BaseIterator;
        friend class RingBuffer;

        using ValueT     = std::conditional_t<kbIsConst, const T, T>;
        using ContainerT = std::conditional_t<kbIsConst, const RingBuffer, RingBuffer>;

    public:
        BaseIterator() = default;

        [[nodiscard]] ValueT& operator*() const
        {
            return m_pContainer->m_items[m_index].value;
        }

        [[nodiscard]] ValueT* operator->() const
        {
            return std::addressof(m_pContainer->m_items[m_index].value);
        }

        BaseIterator& operator++()
        {
            JUG_ASSERT(m_pContainer, "Iterator is not initialized.");
            m_index = (m_index + 1) % m_pContainer->m_items.size();
            return *this;
        }

        BaseIterator operator++(
            const int)
        {
            BaseIterator tmp = *this;
            ++(*this);
            return tmp;
        }

        bool operator==(
            const BaseIterator& _other) const
        {
            return m_pContainer == _other.m_pContainer && m_index == _other.m_index;
        }

        bool operator!=(
            const BaseIterator& _other) const
        {
            return !(*this == _other);
        }

        [[nodiscard]] operator BaseIterator<true>() const
            requires(!kbIsConst)
        {
            return BaseIterator<true> { m_pContainer, m_index };
        }

    protected:
        constexpr BaseIterator(
            ContainerT*  _pOwner,
            const size_t _index)
            : m_pContainer(_pOwner)
            , m_index(_index)
        {
            JUG_ASSERT(_pOwner, "RingBuffer must not be null.");
        }

        ContainerT* m_pContainer = nullptr;
        size_t      m_index      = 0;
    };

    using Iterator      = BaseIterator<false>;
    using ConstIterator = BaseIterator<true>;

public:
    RingBuffer() = default;

    // _capacity: 담고 싶은 원소 개수(GetMaxSize()가 이 값이 됨). 내부적으로 +1 슬롯을 할당.
    explicit RingBuffer(
        const size_t _capacity)
        : m_items(_capacity + 1)
    {
    }

    RingBuffer(
        std::initializer_list<T> _list)
        : m_items(_list.size() + 1)
    {
        for (const T& value: _list)
        {
            Emplace(value);
        }
    }

    RingBuffer(
        const RingBuffer& _other)
        : m_items(_other.m_items.size())
        , m_front(_other.m_front)
        , m_back(_other.m_back)
    {
        for (size_t i = 0; i < _other.GetSize(); ++i)
        {
            const size_t index = (m_front + i) % m_items.size();
            std::construct_at(std::addressof(m_items[index].value), _other.m_items[index].value);
        }
    }

    RingBuffer(
        RingBuffer&& _other) noexcept
        : m_items(std::move(_other.m_items))
        , m_front(std::exchange(_other.m_front, 0))
        , m_back(std::exchange(_other.m_back, 0))
    {
    }

    ~RingBuffer()
    {
        Clear();
    }

    RingBuffer& operator=(
        std::initializer_list<T> _list)
    {
        JUG_ASSERT(_list.size() <= GetMaxSize(), "initializer_list size exceeds capacity.");

        Clear();
        for (const T& value: _list)
        {
            Emplace(value);
        }

        return *this;
    }

    RingBuffer& operator=(
        const RingBuffer& _other)
    {
        if (this != &_other)
        {
            Clear();

            if (m_items.size() != _other.m_items.size())
            {
                m_items.resize(_other.m_items.size());
            }

            m_front = _other.m_front;
            m_back  = _other.m_back;

            for (size_t i = 0; i < _other.GetSize(); ++i)
            {
                const size_t index = (m_front + i) % m_items.size();
                std::construct_at(std::addressof(m_items[index].value), _other.m_items[index].value);
            }
        }

        return *this;
    }

    RingBuffer& operator=(
        RingBuffer&& _other) noexcept
    {
        if (this != &_other)
        {
            Clear();

            m_items = std::move(_other.m_items);
            m_front = std::exchange(_other.m_front, 0);
            m_back  = std::exchange(_other.m_back, 0);
        }

        return *this;
    }

    void Push(
        const T& _value)
    {
        Emplace(_value);
    }

    void Push(
        T&& _value)
    {
        Emplace(std::move(_value));
    }

    template<typename... TArgs>
    T& Emplace(TArgs&&... _args)
    {
        JUG_ASSERT(!IsFull(), "RingBuffer is full.");
        ITEM& item = m_items[m_back];
        std::construct_at(std::addressof(item.value), std::forward<TArgs>(_args)...);
        m_back = (m_back + 1) % m_items.size();
        return item.value;
    }

    [[nodiscard]] T& Front()
    {
        JUG_ASSERT(!IsEmpty(), "RingBuffer is empty.");
        return m_items[m_front].value;
    }

    [[nodiscard]] const T& Front() const
    {
        JUG_ASSERT(!IsEmpty(), "RingBuffer is empty.");
        return m_items[m_front].value;
    }

    void Pop()
    {
        JUG_ASSERT(!IsEmpty(), "RingBuffer is empty.");
        std::destroy_at(std::addressof(m_items[m_front].value));
        m_front = (m_front + 1) % m_items.size();
    }

    void Clear()
    {
        for (size_t i = 0; i < GetSize(); ++i)
        {
            const size_t index = (m_front + i) % m_items.size();
            std::destroy_at(std::addressof(m_items[index].value));
        }
        m_front = 0;
        m_back  = 0;
    }

    // 용량을 재조정. 늘리거나(성장) 줄일(축소) 수 있음. 단 현재 담긴 원소 수보다
    // 작게는 줄일 수 없음. 내부적으로 재할당 + 기존 원소 이동.
    void Reserve(
        const size_t _capacity)
    {
        JUG_ASSERT(_capacity >= GetSize(), "Reserve capacity must not be smaller than current size.");

        if (_capacity == GetMaxSize())
        {
            return;
        }

        std::vector<ITEM> newItems(_capacity + 1);
        const size_t       size = GetSize();
        for (size_t i = 0; i < size; ++i)
        {
            const size_t oldIndex = (m_front + i) % m_items.size();
            std::construct_at(std::addressof(newItems[i].value), std::move(m_items[oldIndex].value));
            std::destroy_at(std::addressof(m_items[oldIndex].value));
        }

        m_items = std::move(newItems);
        m_front = 0;
        m_back  = size;
    }

    [[nodiscard]] bool IsEmpty() const
    {
        return m_front == m_back;
    }

    [[nodiscard]] bool IsFull() const
    {
        return m_items.empty() || (m_back + 1) % m_items.size() == m_front;
    }

    [[nodiscard]] size_t GetSize() const
    {
        return m_items.empty() ? 0 : (m_back + m_items.size() - m_front) % m_items.size();
    }

    [[nodiscard]] size_t GetMaxSize() const
    {
        return m_items.empty() ? 0 : m_items.size() - 1;
    }

    [[nodiscard]] Iterator Begin()
    {
        return Iterator { this, m_front };
    }

    [[nodiscard]] Iterator End()
    {
        return Iterator { this, m_back };
    }

    [[nodiscard]] ConstIterator Begin() const
    {
        return ConstIterator { this, m_front };
    }

    [[nodiscard]] ConstIterator End() const
    {
        return ConstIterator { this, m_back };
    }

    [[nodiscard]] ConstIterator CBegin() const
    {
        return ConstIterator { this, m_front };
    }

    [[nodiscard]] ConstIterator CEnd() const
    {
        return ConstIterator { this, m_back };
    }

    // ===========================================
    //  STL Like
    // ===========================================

    using iterator       = Iterator;
    using const_iterator = ConstIterator;

    [[nodiscard]] size_t size() const
    {
        return GetSize();
    }

    [[nodiscard]] bool empty() const
    {
        return IsEmpty();
    }

    [[nodiscard]] iterator begin()
    {
        return Begin();
    }

    [[nodiscard]] iterator end()
    {
        return End();
    }

    [[nodiscard]] const_iterator begin() const
    {
        return Begin();
    }

    [[nodiscard]] const_iterator end() const
    {
        return End();
    }

    [[nodiscard]] const_iterator cbegin() const
    {
        return CBegin();
    }

    [[nodiscard]] const_iterator cend() const
    {
        return CEnd();
    }

private:
    std::vector<ITEM> m_items = {};
    size_t             m_front = 0;
    size_t             m_back = 0;
};

}   // namespace jug
